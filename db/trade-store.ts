...
export type ImportRun = { id: string; district: string; month: string; kind: Feed; expected_count: number; fetched_at: string; created_at: string; committed: number; mapping_version: string };
...
export async function startImport(db: D1Database, input: { id: string; district: string; month: string; kind: Feed; expectedCount: number; fetchedAt: string }) {
  ...
  await db.prepare("INSERT OR IGNORE INTO trade_import_runs (id, district, month, kind, expected_count, fetched_at, created_at, mapping_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?)")
    .bind(input.id, input.district, input.month, input.kind, input.expectedCount, input.fetchedAt, new Date().toISOString(), COMPLEX_SEED_VERSION).run();
  const run = await runById(db, input.id);
  if (run.district !== input.district || run.month !== input.month || run.kind !== input.kind ||
      run.expected_count !== input.expectedCount || run.fetched_at !== input.fetchedAt || run.mapping_version !== COMPLEX_SEED_VERSION) {
...
export async function appendImport(db: D1Database, id: string, offset: number, input: unknown[]) {
  ...
  if (run.mapping_version !== COMPLEX_SEED_VERSION) throw new ImportError("단지 마스터가 변경되었습니다. 새 수집 작업을 시작해 주세요.");
  ...
  const inserted = await db.prepare(
    ...
    "FROM trade_import_runs r, json_each(?) j WHERE r.id = ? AND r.committed = 0 AND r.mapping_version = ?")
    .bind(hash, JSON.stringify(rows), id, COMPLEX_SEED_VERSION).run();
...
export async function commitImport(db: D1Database, id: string) {
  ...
  await db.batch([
    db.prepare("UPDATE trade_import_runs SET committed = 1 WHERE id = ? AND expected_count = (SELECT COUNT(*) FROM trade_import_rows WHERE run_id = ?)").bind(id, id),
    db.prepare("INSERT OR IGNORE INTO trade_import_prices (run_id, complex_id, area, price_manwon, date) " +
      ...
      "WHERE r.id = ? AND r.committed = 1 AND o.type = 'sale' AND o.complex_id IS NOT NULL) WHERE position = 1").bind(id),
    db.prepare("INSERT INTO trade_snapshot_heads (district, month, kind, run_id, fetched_at, record_count) " +
      "SELECT district, month, kind, id, fetched_at, expected_count FROM trade_import_runs WHERE id = ? AND committed = 1 " +
      ...
      "WHERE excluded.fetched_at > trade_snapshot_heads.fetched_at").bind(id),
  ]);
  const committed = await runById(db, id);
  if (!committed.committed) throw new ImportError("전체 건수 적재가 끝나지 않았습니다. 기존 자료를 유지합니다.");
  ...
  // Scope-bounded cleanup. Never remove the active head or recent staged work.
  const cutoff = new Date(Date.now() - 86400000).toISOString();
  await db.prepare("DELETE FROM trade_import_runs WHERE district = ? AND month = ? AND kind = ? AND created_at < ? AND id <> ? AND id NOT IN (SELECT run_id FROM trade_snapshot_heads)")
    .bind(run.district, run.month, run.kind, cutoff, id).run();
