...
import { readStoredAreaSales } from "../../../db/trade-store";
...
export async function GET(request: Request) {
  ...
  if (!validRecordMonth(month, "sale")) return Response.json({ message: "조회 월이 올바르지 않습니다." }, { status: 400 });
  const d1 = getD1OrNull();
  try {
    if (!d1) throw new Error("DB unavailable");
    return Response.json({ mode: "stored", summaries: await readStoredAreaSales(d1, month),
